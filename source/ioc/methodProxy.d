module ioc.methodProxy;

import std.traits;
import std.typecons;

interface Interceptor(Inter, string fooName) if (is(Inter == interface)) {
...
template Proxy(Impl, TheInterceptor) {
	//template Proxy(Impl, TheInterceptor, string foo, theInterface) {
	//template Proxy(Impl: theInterface, TheInterceptor: Interceptor!(theInterface, foo), string foo, theInterface) {
	...
	string mixinText(string interceptorInstanceName, string resultName, string throwableName){
	...
	class ProxyImpl: Impl {
		auto _interceptorInstance = new TheInterceptor();
		version(unittest) {
			pragma(msg, "------------------------------------");
			pragma(msg, mixinText("_interceptorInstance", "_result", "_throwable"));
			pragma(msg, "====================================");
		}
...
unittest {
	...
	void beforeAfterTests(){
		auto beforeAfterVoid = new Proxy!(TestImplementation, BeforeAfterVoid)();
		beforeAfterVoid.foo();
		assert(LogEntries.entries == ["foo BEFORE", "foo()", "foo AFTER"]);
		LogEntries.reset();

		auto beforeAfterReturned = new Proxy!(TestImplementation, BeforeAfterReturned)();
	...
	void scopeNoParamsTests(){
		auto scopeSuccessNoParams = new Proxy!(TestImplementation, ScopeSuccessNoParams)();
		scopeSuccessNoParams.foo();
		assert(LogEntries.entries == ["foo()", "Success"]);
		LogEntries.reset();

		auto scopeExitNoParams = new Proxy!(TestImplementation, ScopeExitNoParams)();
		scopeExitNoParams.foo();
		assert(LogEntries.entries == ["foo()", "Exit null"]);
		LogEntries.reset();

		auto scopeFailureNoParams = new Proxy!(TestImplementation, ScopeFailureNoParams)();
	...
	void scopeParamsTests(){
		auto scopeSuccessParams = new Proxy!(TestImplementation, ScopeSuccessParams)();
		scopeSuccessParams.bar(2);
		assert(LogEntries.entries == ["bar(2)", "Success 2"]);
		LogEntries.reset();
		
		auto scopeExitParams = new Proxy!(TestImplementation, ScopeExitParams)();
		scopeExitParams.bar(2);
		assert(LogEntries.entries == ["bar(2)", "Exit 2 null"]);
		LogEntries.reset();
		
		auto scopeFailureParams = new Proxy!(TestImplementation, ScopeFailureParams)();
	...
	void hijackingTest(){
		auto hijackingVoid = new Proxy!(TestImplementation, ExceptionHijackVoid)();
		hijackingVoid.baz();
		assert(LogEntries.entries == ["baz()", "hijacked void"]);
		LogEntries.reset();

		auto hijackingNonVoid = new Proxy!(TestImplementation, ExceptionHijackNonVoid)();
